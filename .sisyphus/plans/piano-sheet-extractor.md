...
### Original Request
대중가요 MP3 음원을 업로드하면 사람들이 취미로 칠 수 있는 정도 수준의 피아노 악보를 추출하는 웹 애플리케이션
...
- [ ] 난이도 조절 (초급/중급/고급) 동작
- [ ] Golden Test: Full 모드 곡 중 90% 이상 **핵심 멜로디 모드 85%** 달성 (Task 16 참조)
    ...
    def hook(d):
        if d['status'] == 'downloading':
            ...
            if total > 0:
                ...
                update_job(job_id, {"progress": job_progress})
...
|-------|----------|----------|----------|
| PENDING | `job.json` | Job 생성 즉시 | job.json 존재 |
...
```python
def update_job(job_dir: Path, updates: dict):
    """원자적 job.json 업데이트"""
...
- **동일 스키마 사용**: 파일 저장 포맷 = API 응답 포맷
- **변환 불필요**: `GET /api/result/{job_id}`는 analysis.json을 그대로 반환
...
- **재시작**: 서버 재시작 시 `/tmp/piano-sheet-jobs/` 스캔하여 `status != completed|failed`인 job은 FAILED로 마킹
...
async def cleanup_expired_jobs():
    ...
    while True:
        ...
        for job_dir in Path("/tmp/piano-sheet-jobs").iterdir():
            job_json = job_dir / "job.json"
            if job_json.exists():
                job = json.loads(job_json.read_text())
                # "Z" 접미사를 "+00:00"으로 변환하여 파싱
                created_str = job["created_at"].replace("Z", "+00:00")
                created = datetime.fromisoformat(created_str)
                if (now - created).total_seconds() > 3600:  # 1시간 경과
                    shutil.rmtree(job_dir, ignore_errors=True)
```

**삭제 정책**:
- 생성 후 1시간 경과한 job 디렉토리 전체 삭제
...
### 정답 데이터셋 형식
각 테스트 곡 디렉토리에 다음 파일 포함:
```
tests/golden/data/song_01/
├── input.mp3              # 입력 오디오
...
**데이터 관리**:
- .gitignore에 `tests/golden/data/` 추가 (저작권 보호)
...
1. **테스트 데이터 준비 스크립트** (`scripts/prepare_golden_data.py`):
   ```python
   """
   ...
   def create_directory_structure():
       for song in REQUIRED_SONGS:
           dir_path = Path(f"tests/golden/data/{song['id']}")
...
2. **README 문서화 내용**:
   ...
   ### 준비 방법
   1. `python scripts/prepare_golden_data.py` 실행 → 디렉토리 구조 생성
   2. 각 곡 디렉토리에 다음 파일 추가:
...
3. **pytest 마커 설정** (`conftest.py`):
   ...
   def golden_data_available():
       ...
       data_dir = Path("tests/golden/data")
...
**Acceptance Criteria**:
- [ ] 테스트 MP3 → MIDI 변환 성공
- [ ] MIDI가 MuseScore에서 열림
- [ ] 3분 곡 처리 시간 < 2분 (아래 측정 조건 참조)
...
**Acceptance Criteria**:
- [ ] YouTube URL → MP3 추출 성공
- [ ] 20분 초과 영상 거부 (다운로드 전 체크, `extract_info()` 사용)
- [ ] 비공개 영상 에러 처리
- [ ] 진행률 콜백 동작 (yt-dlp 내부 0-100% → Job progress 0-20%로 변환)
...
**Acceptance Criteria**:
- [ ] `notes_to_stream()` 함수 구현 완료
- [ ] `stream_to_musicxml()` 함수 구현 완료
- [ ] `notes_to_musicxml()` 편의 함수 구현 완료
...
- **지원 코드**: 12개 루트 × 2 (major/minor) = 24개만 지원
- **미지원**: 7th, 9th, sus, dim, aug 등 복잡한 코드
- **정규화**: 입력 시 대소문자 무관, 저장 시 "C", "Cm" 형식으로 통일
...
**Acceptance Criteria**:
- [ ] BPM 감지 ±5 BPM 정확도 (알려진 BPM 곡 3개로 테스트)
- [ ] Key 감지 (major/minor 구분)
- [ ] 코드 진행 리스트 출력 (시간, 코드명, 신뢰도)
- [ ] 코드 정확도 기대치: ~60% (사용자 수동 수정 전제)
...
- [ ] 3가지 난이도 MusicXML 생성
- [ ] 초급: 음표 수 ≤ 고급의 50%
- [ ] 코드 심볼이 악보에 표시됨 (analysis.json 기반)
...
**Acceptance Criteria**:
- [ ] POST /api/upload → job_id 반환
- [ ] POST /api/youtube → job_id 반환
- [ ] GET /api/status/{job_id} → progress 0-100%
- [ ] GET /api/result/{job_id} → 분석 결과 + 다운로드 URL
- [ ] PUT /api/result/{job_id} → 수정 후 재생성
- [ ] GET /api/download/{job_id}/{format} → 파일 다운로드
...
- [ ] **프록시 검증**: 아래 curl 명령으로 멀티파트 업로드가 프록시를 통해 백엔드로 전달되는지 확인
  ...
  curl -X POST http://localhost:3000/api/upload \
    -F "file=@tests/fixtures/test_audio.mp3" \
    -v
...
### Task 14: Golden Test 시스템 구축

**What to do**:
- 비교 알고리즘 구현 (상단 "Golden Test Comparison Algorithm" 참조)
- pytest 통합
- 리포트 생성 (JSON + HTML)
- 테스트 데이터 구조 설정
...
*생성할 파일:*
- `tests/golden/comparator.py` - Note 비교 알고리즘
- `tests/golden/test_golden.py` - pytest 테스트 케이스
- `tests/golden/conftest.py` - pytest fixtures

*참조할 계획서 섹션:*
- "Golden Test Comparison Algorithm" - 비교 알고리즘 상세, Pass/Fail 기준
...
**Commit**: `feat(tests): implement Golden Test comparison system`
...
*생성할 파일:*
- `scripts/generate_test_audio.py` - 테스트용 오디오 생성
- `docs/e2e-test-guide.md` - E2E 테스트 실행 가이드
...
**What to do**:
- 10곡 테스트 데이터 준비 (MP3 + reference.mid + metadata.json)
- Golden Test 전체 실행
- 실패 분석 및 파라미터 튜닝
- 핵심 멜로디 모드 85% 달성까지 반복

**Parallelizable**: NO (Task 14, 15 필요)

---

## Golden Test 데이터 유연성 정책

> **핵심 원칙**: Reference MIDI가 없어도 테스트가 가능해야 합니다.

### 테스트 데이터 3가지 모드

| 모드 | 필요 파일 | 검증 방식 | 용도 |
|------|----------|----------|------|
| **Full** | MP3 + reference.mid + metadata.json | 정량 비교 (85% 기준) | 정확도 측정 |
| **Smoke** | MP3 + metadata.json (MIDI 없음) | 처리 성공 여부만 확인 | 기능 동작 확인 |
| **Manual** | MP3만 | 수동 청취 검증 | 빠른 확인 |

### 테스트 데이터 구조 (확장)

```
tests/golden/data/
├── song_01/                    # Full 모드 (정량 테스트)
│   ├── input.mp3
│   ├── reference.mid           # 있음
│   └── metadata.json
├── song_02/                    # Smoke 모드 (MIDI 없음)
│   ├── input.mp3
│   ├── reference.mid           # 없음 (또는 빈 파일)
│   └── metadata.json           # has_reference: false
└── song_03/                    # Manual 모드
    └── input.mp3               # metadata도 없음
```
...
```json
{
  ...
  "duration_seconds": 180,
  "source": "manual_transcription",
  "has_reference": true,         // false면 Smoke 모드
  "skip_reason": null            // 스킵 사유 (있으면 테스트 제외)
}
```
...
def test_golden_song(song_dir: Path):
    ...
    # 2. 메타데이터 로드 (없으면 Manual 모드)
    if metadata_path.exists():
        metadata = json.loads(metadata_path.read_text())
        if metadata.get("skip_reason"):
            pytest.skip(metadata["skip_reason"])
    else:
        metadata = {"title": song_dir.name, "has_reference": False}
    
    # 3. 처리 실행
    result = process_audio(input_path)
    assert result.status == "completed", f"처리 실패: {result.error}"
    
    # 4. Reference MIDI 유무에 따른 검증
    has_reference = metadata.get("has_reference", True) and reference_path.exists()
    ...
    if has_reference:
        ...
        assert comparison.passed, (
            f"Golden Test 실패: {metadata['title']}\n"
            f"Core similarity: {comparison.core_similarity:.1f}% (필요: 85%)"
        )
    else:
        # Smoke 모드: 출력 파일 존재 확인만
        ...
        assert len(notes) > 0, "추출된 음표가 없음"
        assert len(notes) < 10000, "비정상적으로 많은 음표"
```

### 테스트 리포트 (MIDI 없는 경우 포함)
  ...
  "results": [
    {
      ...
      "mode": "full",
      "passed": true,
      "core_similarity": 87.5
    },
    {
      "song": "song_02",
      "mode": "smoke",
      "passed": true,
      "note": "Reference MIDI 없음, 처리 성공 확인만"
    }
  ]
}
```
...
> Reference MIDI가 없는 곡은 "처리가 되는가"만 확인하므로, 
> 정확도 측정에서는 제외됩니다.

---

**Acceptance Criteria**:
- [ ] 테스트 데이터 준비 (Full 모드 최소 5곡, Smoke 모드 허용)
- [ ] Full 모드 곡 중 90% 이상 **핵심 멜로디 모드 85%** 달성
- [ ] Smoke 모드 곡 전체 처리 성공
- [ ] 튜닝된 파라미터 문서화 (config.py에 기록)
- [ ] 실패 곡 분석 리포트 작성

**Commit**: `feat(tests): tune algorithm parameters for 85% core melody accuracy`
...
- [ ] Golden Test 9/10곡 이상 **핵심 멜로디 모드 85%** 달성
- [ ] Docker 원클릭 실행
- [ ] README 문서화 완료
